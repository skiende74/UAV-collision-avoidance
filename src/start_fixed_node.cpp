...
int main(int argc, char **argv)
{
    ...
    double L,L20,L21,L2;
    L = 100.0;                   // 회전익 출발지점에서 충돌지점까지거리(로컬)
    L20 = 50.0;                 //고정익 이륙거리 50
    L21 =  170.0 + 200.0;       // 고정익
    L2=L20+L21;          //시작지점에서 충돌지점까지거리(로컬,고정익) 280:회전익 이륙까지걸리는시간기다리기위함
    double Z = 5.0;                    // 충돌높이
    
    double th[N], type[N];              //to set direction. TYPE : 회전익 1 고정익 2
    th[0] = M_PI;
    th[1] = 3.0/2.0*M_PI;
    type[0] = 2;
    type[1] = 1;
    ...
    double x_s[N], y_s[N], z_s[N], x_s2[N], y_s2[N], z_s2[N]; //시작점 in global, 고정익 TAKEOFF점
    double x_wp[N], y_wp[N], z_wp[N];                         //wp in global
    double x_loit[N], y_loit[N], z_loit[N];                   // loiter point in global
    double x_col0, y_col0, z_col;                               //충돌지점 in global
    double x_col,y_col, x_offset[N],y_offset[N];
...
*/
    ...
    for (i = 0; i < N; i++)
    {
        x_col=x_col0 + x_offset[i]*ltogx;
        y_col=y_col0 + y_offset[i]*ltogy;
        if (type[i] == 1)
        {
        ...
        {
            dx = L2 * cos(th[i]) * ltogx;
            dy = L2 * sin(th[i]) * ltogy;

            x_s[i] = x_col + dx;
            y_s[i] = y_col + dy;
            z_s[i] = z_col;

            x_s2[i] = x_col + L21 / L2 * dx;
            y_s2[i] = y_col + L21 / L2 * dy;
            z_s2[i] = z_col;
            ...
            x_loit[i] = x_col + 2.0 * dx + R_loit*cos(th[i]+M_PI_2)*ltogx; // LOITER 중심위치계산.
            y_loit[i] = y_col + 2.0 * dy + R_loit*cos(th[i]+M_PI_2)*ltogy;
            z_loit[i] = z_col;
        }
    ...
    for (i = 0; i < N && ros::ok(); i++)
    {
        ...
        else if (type[i] == 2)
        {
            ...
            msg[i].param3 = 0.0;
            msg[i].param4 = 0.0;
            msg[i].x_lat = y_loit[i];
            msg[i].y_long = x_loit[i];
            msg[i].z_alt = z_loit[i];
